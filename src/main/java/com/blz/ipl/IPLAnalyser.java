...
public class IPLAnalyser {
	...
	public List<String> BestBattingAverageWithBestBowlingAverage() throws IOException, IPLAnalyserException {
		...
		List<IPLBattingAnalysis> playerWithBestBattingAvg = iplRunsCSVList.stream()
		...
		List<IPLBowlingAnalysis> playerWithBestBowlingAvg = iplWktsCSVList.stream()
				.sorted(Comparator.comparingDouble(player -> player.avg)).collect(Collectors.toList());
		for (IPLBattingAnalysis mostRunsCSV : playerWithBestBattingAvg) {
			for (IPLBowlingAnalysis mostWktsCSV : playerWithBestBowlingAvg) {
				if (mostRunsCSV.playerName.equals(mostWktsCSV.playerName)) {
					list.add(mostRunsCSV.playerName);
				}
			}
		}
		return list;
	...
	public List<IPLBattingAnalysis> maximum100sWithBestBattingAverage(String csvFilePath)
			throws IOException, IPLAnalyserException {
		loadCSVData(csvFilePath);
		int century = iplRunsCSVList.stream().map(IPLBattingAnalysis::getCenturies).max(Integer::compare).get();
		List<IPLBattingAnalysis> maxCentury = iplRunsCSVList.stream().filter(player -> player.getCenturies() == century)
				.collect(Collectors.toList());
		double battingAvg = maxCentury.stream().map(IPLBattingAnalysis::getAvg).max(Double::compare).get();
		return maxCentury.stream().filter(player -> player.getAvg() == battingAvg).collect(Collectors.toList());
	}

	public List<IPLBattingAnalysis> getWhoHitZero100sAnd50sButBestBattingAvgs(String csvFilePath)
			throws IOException, IPLAnalyserException {
		loadCSVData(csvFilePath);
		List<IPLBattingAnalysis> playerWithout100sAnd50s = iplRunsCSVList.stream()
				.filter(player -> (player.getCenturies() + player.getFifties()) == 0).collect(Collectors.toList());
		double playerWithBestAvg = playerWithout100sAnd50s.stream().map(IPLBattingAnalysis::getAvg).max(Double::compare)
				.get();
